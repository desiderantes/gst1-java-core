 ...
 * <p>
 * <pre>proportion</pre> indicates the real-time performance of the streaming in
 ...
 * producing data fast enough. 1.0 is the ideal <pre>proportion</pre> value. The
 * proportion value can safely be used to lower or increase the quality of the
 * element.
 * <p>
 * <pre>difference</pre> is the difference against the clock in running time of
 * the last buffer that caused the element to generate the QOS event. A negative
 * value means that the buffer with <pre>timestamp</pre> arrived in time. A
 * positive value indicates how late the buffer with <pre>timestamp</pre> was.
 * <p>
 * <pre>timestamp</pre> is the timestamp of the last buffer that cause the element
 * to generate the QOS event. It is expressed in running time and thus an ever-increasing value.
 * <p>
 * The upstream element can use the <pre>diff</pre> and <pre>timestamp</pre> values
 * to decide whether to process more buffers. For positive <pre>difference</pre>,
 * all buffers with timestamp &le; <pre>timestamp</pre> + <pre>difference</pre> will
