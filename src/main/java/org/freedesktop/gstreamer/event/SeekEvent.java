 ...
 * <p>
 * The seek event configures playback of the pipeline between <pre>start</pre> to <pre>stop</pre>
 * at the speed given in <pre>rate</pre>, also called a playback segment.
 * <p>
 * The <pre>start</pre> and <pre>stop</pre> values are expressed in <pre>format</pre>.
 * <p>
 * A <pre>rate</pre> of 1.0 means normal playback rate, 2.0 means double speed.
 ...
 * <p>
 * <pre>startType</pre> and <pre>stopType</pre> specify how to adjust the currently configured
 * start and stop fields in <pre>segment</pre>. Adjustments can be made relative or
 * absolute to the last configured values. A type of {@link SeekType#NONE} means
 * that the position should not be updated.
 * <p>
 * When the rate is positive and <pre>start</pre> has been updated, playback will start
 * from the newly configured start position.
 * <p>
 * For negative rates, playback will start from the newly configured <pre>stop</pre>
 * position (if any). If the stop position if updated, it must be different from
 * -1 for negative rates.
 * <p>
 * It is not possible to seek relative to the current playback position, to do
 * this, PAUSE the pipeline, query the current playback position with
 * {@link org.freedesktop.gstreamer.Pipeline#queryPosition getPosition} and update the playback segment
