 ...
 *
 * @see <a href="https://cgit.freedesktop.org/gstreamer/gstreamer/tree/libs/gst/base/gstbasesink.h?h=1.8"></a>
 */
public interface BaseSinkAPI extends Library {
    ...
    interface GetCaps extends Callback {
        Caps callback(BaseSink sink, Caps caps);
    }

    interface SetCaps extends Callback {
        boolean callback(BaseSink sink, Caps caps);
    }
    ...
    interface ActivatePull extends Callback {
        boolean callback(BaseSink sink, boolean active);
    }

    interface GetTimes extends Callback {
        void callback(BaseSink sink, Buffer buffer,
                             Pointer /* GstClockTime */ start, Pointer /* GstClockTime */ end);
    }

    interface ProposeAllocation extends Callback {
        boolean callback(BaseSink sink, AllocationQuery query);
    }

    interface BooleanFunc1 extends Callback {
        boolean callback(BaseSink sink);
    }

    interface QueryNotify extends Callback {
        boolean callback(BaseSink sink, Query query);
    }
    ...
    interface WaitEventNotify extends Callback {
        FlowReturn callback(BaseSink sink, Event event);
    }

    interface Render extends Callback {
        FlowReturn callback(BaseSink sink, Buffer buffer);
    }

    interface RenderList extends Callback {
        FlowReturn callback(BaseSink sink, GList bufferList);
    }

    final class GstBaseSinkStruct extends com.sun.jna.Structure {
    ...
    final class GstBaseSinkClass extends com.sun.jna.Structure {
        //
        // Actual data members
        //
