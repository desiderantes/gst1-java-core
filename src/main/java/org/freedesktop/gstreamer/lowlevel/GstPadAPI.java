...
public interface GstPadAPI extends com.sun.jna.Library {
    int GST_PAD_PROBE_TYPE_INVALID = 0;
    /* flags to control blocking */
    int GST_PAD_PROBE_TYPE_IDLE = (1 << 0);
    int GST_PAD_PROBE_TYPE_BLOCK = (1 << 1);
    /* flags to select datatypes */
    int GST_PAD_PROBE_TYPE_BUFFER = (1 << 4);
    int GST_PAD_PROBE_TYPE_BUFFER_LIST = (1 << 5);
    int GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM = (1 << 6);
    int GST_PAD_PROBE_TYPE_EVENT_UPSTREAM = (1 << 7);
    int GST_PAD_PROBE_TYPE_EVENT_FLUSH = (1 << 8);
    int GST_PAD_PROBE_TYPE_QUERY_DOWNSTREAM = (1 << 9);
    int GST_PAD_PROBE_TYPE_QUERY_UPSTREAM = (1 << 10);
    /* flags to select scheduling mode */
    int GST_PAD_PROBE_TYPE_PUSH = (1 << 12);
    int GST_PAD_PROBE_TYPE_PULL = (1 << 13);
    int GST_PAD_PROBE_TYPE_BLOCKING = GST_PAD_PROBE_TYPE_IDLE | GST_PAD_PROBE_TYPE_BLOCK;
    int GST_PAD_PROBE_TYPE_DATA_DOWNSTREAM = GST_PAD_PROBE_TYPE_BUFFER | GST_PAD_PROBE_TYPE_BUFFER_LIST | GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM;
    int GST_PAD_PROBE_TYPE_BLOCK_DOWNSTREAM = GST_PAD_PROBE_TYPE_BLOCK | GST_PAD_PROBE_TYPE_DATA_DOWNSTREAM;
    int GST_PAD_PROBE_TYPE_DATA_UPSTREAM = GST_PAD_PROBE_TYPE_EVENT_UPSTREAM;
    int GST_PAD_PROBE_TYPE_DATA_BOTH = GST_PAD_PROBE_TYPE_DATA_DOWNSTREAM | GST_PAD_PROBE_TYPE_DATA_UPSTREAM;
    int GST_PAD_PROBE_TYPE_BLOCK_UPSTREAM = GST_PAD_PROBE_TYPE_BLOCK | GST_PAD_PROBE_TYPE_DATA_UPSTREAM;
    int GST_PAD_PROBE_TYPE_EVENT_BOTH = GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM | GST_PAD_PROBE_TYPE_EVENT_UPSTREAM;
    int GST_PAD_PROBE_TYPE_QUERY_BOTH = GST_PAD_PROBE_TYPE_QUERY_DOWNSTREAM | GST_PAD_PROBE_TYPE_QUERY_UPSTREAM;
    int GST_PAD_PROBE_TYPE_ALL_BOTH = GST_PAD_PROBE_TYPE_DATA_BOTH | GST_PAD_PROBE_TYPE_QUERY_BOTH;
    int GST_PAD_PROBE_TYPE_SCHEDULING = GST_PAD_PROBE_TYPE_PUSH | GST_PAD_PROBE_TYPE_PULL;
    ...
    @CallerOwnsReturn
    Pointer ptr_gst_pad_new(String name, PadDirection direction);

    @CallerOwnsReturn
    Pointer ptr_gst_pad_new_from_template(PadTemplate templ, String name);
...
//    }
    ...
    interface PadFixateCaps extends GstCallback {
        void callback(Pad pad, Caps caps);
    }

    interface PadProbeCallback extends GstCallback {
        PadProbeReturn callback(Pad pad, GstPadProbeInfo probeInfo, Pointer user_data);
    }
