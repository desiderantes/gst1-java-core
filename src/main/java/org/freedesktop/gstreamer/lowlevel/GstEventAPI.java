 ...
 *
 * @see <a href="https://cgit.freedesktop.org/gstreamer/gstreamer/tree/gst/gstevent.h?h=1.8"></a>
 */
public interface GstEventAPI extends com.sun.jna.Library {
    ...
    /* newsegment events */
    @CallerOwnsReturn
    Event gst_event_new_segment(GstAPI.GstSegmentStruct segmentStruct);
    ...
    /* QOS events */
    @CallerOwnsReturn
    Event gst_event_new_qos(QOSType type, double proportion, long diff, long timestamp);
    ...
    /* seek event */
    @CallerOwnsReturn
    Event gst_event_new_seek(double rate, Format format, int flags,
    ...
    /* navigation event */
    @CallerOwnsReturn
    Event gst_event_new_navigation(@Invalidate Structure structure);

    Pointer ptr_gst_event_new_navigation(@Invalidate Structure structure);

    /* latency event */
    @CallerOwnsReturn
    Event gst_event_new_latency(long latency);
    ...
    /* step event */
    @CallerOwnsReturn
    Event gst_event_new_step(Format format, long amount, double rate, boolean flush, boolean intermediate);
    ...
    /* stream start event */
    @CallerOwnsReturn
    Event gst_event_new_stream_start(final String stream_id);
     ...
     * A #GstEvent.
     */
    final class EventStruct extends com.sun.jna.Structure {
        ...
        protected List<String> getFieldOrder() {
            return Arrays.asList("mini_object",
