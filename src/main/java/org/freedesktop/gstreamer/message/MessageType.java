...
public enum MessageType implements NativeEnum<MessageType> {
    ...
    /**
     * Message indicating a {@link org.freedesktop.gstreamer.glib.GObject} property has changed (Since 1.10)
     */
    @Gst.Since(minor = 10)
    PROPERTY_NOTIFY(EXTENDED.intValue() + 3),
    /**
     * Message indicating a new GstStreamCollection is available (Since
     * 1.10)
     */
    @Gst.Since(minor = 10)
    STREAM_COLLECTION(EXTENDED.intValue() + 4),
    /**
     * Message indicating the active selection of GstStreams has changed
     * (Since 1.10)
     */
    @Gst.Since(minor = 10)
