...
public class PadTest {
    ...
    public void addEventProbe() {
        ...
        Pad.EVENT_PROBE event_probe = (pad, event) -> {
            e.set(event);
    ...
    public void addDataProbe() {
        ...
        Pad.DATA_PROBE data_probe = (pad, buffer) -> {
            b.set(buffer);
            return PadProbeReturn.OK;
        };
    ...
    public void addProbe_Query() {
        ProbeTester.test(PadProbeType.QUERY_BOTH, info -> {
            assertTrue(info.getType().contains(PadProbeType.QUERY_DOWNSTREAM)
                            || info.getType().contains(PadProbeType.QUERY_UPSTREAM),
                    "Info type does not include query");
