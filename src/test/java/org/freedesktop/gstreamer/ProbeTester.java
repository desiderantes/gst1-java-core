 ...
 * <p>
 * Call {@link ProbeTester#test(String, Set, Predicate)} and pass a callback which will
...
public class ProbeTester {
    ...
    public static void test(String pipeline, Set<PadProbeType> mask, Predicate<PadProbeInfo> callback) {
        ...
        if (mask.contains(PadProbeType.QUERY_DOWNSTREAM) || mask.contains(PadProbeType.QUERY_UPSTREAM)) {
            // Trigger at least one query so QUERY probes do not depend on pipeline internals.
            sink.query(new DurationQuery(Format.TIME));
        }
